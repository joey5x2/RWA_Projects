...
def build_tools(df_old, df_new, merged, input_diff, driver_summary):
    ...
    @tool
    def code_text() -> str:
        """Return the Python source code for the CalcEAD class."""
        return inspect.getsource(rwa_calc.CalcEAD)

    return [get_netting_set_summary, get_changed_rows, get_driver_summary, preview_data, code_text]
...
def ai_agent(tools,llm_model):
    ...
    - Illiquid Flag and LRM flag are totally different. Do not confuse the two.
