...
class Trade:
    ...
    def get_trade_addon(self):
        ...
        print(f"trade id: {self.trade_id} -> adj notional: {adjusted_notional}; maturity factor: {mf}; supervisory delta: {sup_delta}; supervisory factor: {sf}, addon: {addon}, model: 'SA-CCR'")
        self.subscriber.post_res({'Type': 'Trade', 'ID': self.trade_id, 'Model':'SA-CCR', 'Underlying_Ticker': self.underlying_ticker, 'Adjusted_Notional': adjusted_notional, 'Maturity_Factor': mf, 'Supervisory_Delta': sup_delta, 'Supervisory_Factor': sf, 'Trade_Addon': addon})
        return addon
...
def Cal_EAD(netting_sets:dict):
    ...
    for tab_name, ns in netting_sets.items():
        ns.subscriber.reset()
        ns.get_EAD()
    ...
    ead_summary = ead_summary[["Type", "ID", "Model", "Replacement_Cost", "PFE_addon", "multiplier", "EAD"]]
...
if __name__ == '__main__':
    all_netting_sets = feed_data_excel('inputs_demo.xlsx')
